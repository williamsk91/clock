...
describe("datetime", () => {
  describe("with start and without time", () => {
    test("should show only date", () => {
      expect(formatDatetime(start, undefined, false, now)).toBe("16 Apr");
    });
    test("should show year if not on this year", () => {
      expect(formatDatetime(addYears(start, 1), undefined, false, now)).toBe(
        "16 Apr 2021"
      );
    });
  });
  describe("with start and with time", () => {
    test("should show date and time", () => {
      expect(formatDatetime(start, undefined, true, now)).toBe("16 Apr 10:00");
    });
    test("should show year if not on this year", () => {
      expect(formatDatetime(addYears(start, 1), undefined, true, now)).toBe(
        "16 Apr 2021 10:00"
      );
    });
  });
  describe("with both start and end, but without time", () => {
    test("should have range in date", () => {
      expect(formatDatetime(start, addDays(start, 3), false, now)).toBe(
        "16 Apr~19 Apr"
      );
    });
  });
  describe("with both start and end, and with time", () => {
    test("should have range in time if on the same date", () => {
      expect(formatDatetime(start, addHours(start, 3), true, now)).toBe(
        "16 Apr 10:00~13:00"
      );
    });
    test("should have range in date if not on the same date", () => {
      expect(formatDatetime(start, addDays(start, 3), true, now)).toBe(
        "16 Apr 10:00~19 Apr 10:00"
      );
    });
  });
});
